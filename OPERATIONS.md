...
| タイプ | 説明 | 用途 |
|--------|------|------|
| 診断タイプ | QRスキャン → 診断開始 | お口年齢診断、矯正チェック等 |
| リンクタイプ | QRスキャン → 任意URLへリダイレクト | 予約ページ、LINE登録等 |
...
**設定方法:**
- QRコード作成時または編集時に「有効期限」を設定
- 空欄の場合は無期限

**期限切れ時の動作:**
- 専用の期限切れページへリダイレクト
- ダッシュボードに「有効期限切れ」ラベル表示
...
| ファイル | 説明 |
|----------|------|
| `/src/app/c/[code]/route.ts` | QRスキャン時のルーティング |
| `/src/app/c/[code]/expired/page.tsx` | 期限切れページ |
| `/src/app/dashboard/channels/new/page.tsx` | QRコード作成画面 |
| `/src/app/api/dashboard/locations/route.ts` | エリアデータAPI |
| `/src/app/api/dashboard/history/route.ts` | 履歴データAPI |
