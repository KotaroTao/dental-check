...
| 診断完了 | 診断を完了したユーザー数 |
| 完了率 | アクセス数に対する診断完了率 |
| CTAクリック | CTAボタンのクリック数 |

**詳細表示:**
- PC: 診断完了にホバーで年齢・性別分布、CTAクリックにホバーで内訳を表示
- スマホ: 「詳細を表示」ボタンをタップで詳細を展開

**画像管理:**
QRコード編集画面（/dashboard/channels/[id]/edit）から画像のアップロード・変更が可能
- ドラッグ&ドロップまたはファイル選択
- 自動圧縮（最大幅800px、JPEG 80%品質）
- 対応形式: JPEG, PNG, GIF, WebP（最大5MB）

#### CTAクリック推移
期間内のCTAクリック統計を表形式で表示：
- 診断完了数
- 合計クリック数
- 診断結果からのクリック
- 医院ページからのクリック

※非表示にしたQRコードのデータは集計から除外されます

#### 診断実施エリア
都道府県別のアクセス分布を地図上に表示

#### 診断完了履歴
直近の診断完了セッション一覧（年齢・性別・CTAクリック状況）
