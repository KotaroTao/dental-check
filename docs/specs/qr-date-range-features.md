...
#### 動作仕様
- ダッシュボードの各QRコードカードにCPA/CPD/CPCを表示
- 既存のソートオプションにCPA/CPD/CPCを追加
- 指標名の横にカッコで補足説明を表示
...
| 項目 | 表示名 | 説明 |
|------|--------|------|
| CPA | CPA（アクセス単価） | 広告費用 / アクセス数 |
| CPD | CPD（診断完了単価） | 広告費用 / 診断完了数 |
| CPC | CPC（CTAクリック単価） | 広告費用 / CTAクリック数 |
...
│ [QRコード名] チラシ①                                        │
│ お口年齢診断 / 2024/01/15                                   │
│ 掲載期間: 2024/01/15〜2024/03/31                            │
├─────────────────────────────────────────────────────────────┤
│ QR読み込み │ 診断完了 │ 完了率 │ CTA │ CPA      │ CPD      │ CPC      │
│ 1,234     │ 890     │ 72.1% │ 234 │ ¥40      │ ¥56      │ ¥213     │
│           │         │       │     │(アクセス) │(診断完了) │(CTAクリック)│
└─────────────────────────────────────────────────────────────┘
```

#### 実装箇所
- `/src/app/dashboard/page.tsx` - CPA/CPD/CPC表示、ソート追加
